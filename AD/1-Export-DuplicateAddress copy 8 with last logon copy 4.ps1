...
$processedUsers = foreach ($user in $users) {
    # Filter out x400 and x500 addresses from ProxyAddresses
    $filteredProxyAddresses = $user.ProxyAddresses -notmatch 'x400|x500'
    ...
    [PSCustomObject]@{
        ...
        ProxyAddresses = $filteredProxyAddresses -join '; '
...
$duplicates = $processedUsers | Where-Object { $_.ProxyAddresses -ne '' } | Group-Object -Property ProxyAddresses | Where-Object { $_.Count -gt 1 }
...
$outputData = foreach ($dup in $duplicates) {
    ...
    foreach ($user in $sortedGroup) {
        if ($user -eq $mostRecentUser) {
            ...
            Write-Host "Most Recent Logon (Green): CN: $($user.CommonName) - ProxyAddresses: $($user.ProxyAddresses) - Most Recent Logon: $($user.MostRecentLogon)" -ForegroundColor Green
        } else {
            # Other duplicates are shown in yellow
            Write-Host "Other Duplicates: CN: $($user.CommonName) - ProxyAddresses: $($user.ProxyAddresses) - Most Recent Logon: $($user.MostRecentLogon)" -ForegroundColor Yellow
        }
...
# Display in GridView
$outputData | Out-GridView -Title "AD Users with Duplicate ProxyAddresses and Recent Logon"
