...
# Function to rename existing user
function Rename-ExistingUser {
    param (
    ...
    try {
        $existingUser = Get-ADUser -Filter {UserPrincipalName -eq $UserPrincipalName}
        if ($existingUser) {
            $newName = $existingUser.Name + $NewSuffix
            Rename-ADObject -Identity $existingUser.DistinguishedName -NewName $newName
            ...
            Set-ADUser -Identity $existingUser `
                ...
                -UserPrincipalName ($existingUser.UserPrincipalName -replace '@', $NewSuffix + '@') `
                -EmailAddressPolicyEnabled $false `
                -EmailAddress ($existingUser.EmailAddress + $NewSuffix)

            Write-Host "Updated DisplayName, UserPrincipalName, and EmailAddresses with suffix '$NewSuffix'." -ForegroundColor Green
        } else {
...
if (-not $mailbox -and -not $mailUser) {
    ...
    if ($existingObject) {
        ...
        if ($existingObject.RecipientType -eq 'MailUser' -or $existingObject.RecipientType -eq 'MailContact' -or $existingObject.RecipientType -eq 'User') {
            Rename-ExistingUser -UserPrincipalName "JHeuser@arnpriorhealth.ca" -NewSuffix "-old"
        } else {
