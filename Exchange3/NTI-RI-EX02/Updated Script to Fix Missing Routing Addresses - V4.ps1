    ...
    try {
        ...
        foreach ($mailbox in $mailboxes) {
            try {
                ...
                # Get current email addresses and convert to strings
                $currentAddresses = @($mailbox.EmailAddresses | ForEach-Object { $_.ToString() })
                ...
                $addressString = '"' + ($currentAddresses -join '","') + '"'
                $updateCommand = "Set-Mailbox -Identity `"$($mailbox.Alias)`" -EmailAddresses @($addressString)"
                ...
                # Pause on error to review
                Write-Host "Press Enter to continue with next mailbox..." -ForegroundColor Yellow
                Read-Host
            }
        }
