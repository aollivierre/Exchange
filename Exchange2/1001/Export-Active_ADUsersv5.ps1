...
$userInfo = $users | ForEach-Object {
    ...
    $primarySMTPs = ($mailUser.EmailAddresses | Where-Object { $_.PrefixString -eq "SMTP" }).SmtpAddress
    $primarySMTP = if ($primarySMTPs -is [array]) { $primarySMTPs[0] } else { $primarySMTPs }
...
$userInfo | Export-Csv -Path "C:\Code\Exchange\Exports\AGH_October_3rd_2023_16_30_PM_AD_Users_PowerShell_Export_V10.csv" -NoTypeInformation
