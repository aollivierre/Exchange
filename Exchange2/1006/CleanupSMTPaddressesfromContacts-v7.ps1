...
function Update-MailContactProxyAddresses {
    ...
    $filteredProxyAddresses = $proxyAddresses | Where-Object {
        # Keep X400 and X500 addresses
        ($_ -like "X400:*") -or ($_ -like "X500:*") -or
        # Keep primary SMTP address (indicated by uppercase SMTP)
        ($_ -cmatch "^SMTP:") -or
        # Keep SMTP addresses not matching any accepted domain
        ($_ -cmatch "^smtp:" -and $acceptedDomains -notcontains $_.Split('@')[1].ToLower())
    }
