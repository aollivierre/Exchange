...
# 3- Read CSV file
$csvFilePath = "C:\Code\CB\Exchange\Glebe\Exports\Glebe_Exchange_Migration_Last_batch_7users_June_14_2023.csv" # Replace with the actual CSV file path
...
foreach ($item in $csvFile) {
    ...
    # $user = Get-ADUser -Filter 'Enabled -eq $true' -Properties DistinguishedName, mail, proxyAddresses | Where-Object { $_.proxyAddresses -like "*$emailAddress*" }
    $user = Get-ADUser -Filter * -Properties DistinguishedName, mail, proxyAddresses | Where-Object { $_.proxyAddresses -like "*$emailAddress*" }
