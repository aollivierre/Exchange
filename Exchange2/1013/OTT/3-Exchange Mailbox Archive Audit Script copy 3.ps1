    ...
    function Convert-ToGB {
        ...
        try {
            if ($sizeString.ToString() -match "\(([0-9,]+) bytes\)") {
                return [math]::Round([decimal]$matches[1].Replace(",","")/1GB, 4) # Increased to 4 decimal places
            }
            return 0
        } catch {
...
$ArchiveserverName = (Get-MailboxDatabase $mailbox.ArchiveDatabase).Server

# Create filename with date and FQDN
$date = Get-Date -Format "yyyyMMdd-HHmmss"
$baseFileName = "ArchiveAudit_{0}_{1}" -f $ArchiveserverName, $date

# Export results to CSV and HTML
$results | Export-CSV "$baseFileName.csv" -NoTypeInformation
$results | Out-HtmlView -FilePath "$baseFileName.HTML" -Title "NTI_onprem_exchange_$ArchiveserverName`_$date"
