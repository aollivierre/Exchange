function Get-ArchiveReport {
    ...
    if (!$ProcessOnly) {
        ...
        $results = Get-Mailbox -ResultSize Unlimited | ForEach-Object {
            ...
            [PSCustomObject]@{
                ...
                PrimaryMailboxItemCount = if ($stats) { $stats.ItemCount } else { 0 }
                ...
                IsEnabled = if ($stats) { $stats.IsValid } else { "Never Logged On" }
    ...
    if (!$CollectOnly) {
        ...
        if (!$QuietMode) {
            ...
            Write-Host "Never Logged On: $(($results | Where-Object {$_.NeverLoggedOn}).Count)"
