    ...
    try {
        ...
        foreach ($mailbox in $mailboxes) {
            try {
                ...
                $newAddressList = @($currentAddresses) + $newAddress
                
                # Update mailbox
                Set-Mailbox -Identity $mailbox.PrimarySmtpAddress -EmailAddresses $newAddressList
        ...
        if ($remainingIssues.Count -eq 0) {
            ...
            $remainingIssues | Format-Table DisplayName, PrimarySmtpAddress -AutoSize
        }
    }
    catch {
        Write-Error "Error: $_"
