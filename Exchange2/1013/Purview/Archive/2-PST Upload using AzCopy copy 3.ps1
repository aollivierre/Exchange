function Start-PSTUpload {
    ...
    # Function to validate a PST file
    ...
    $pstFiles = switch ($PSCmdlet.ParameterSetName) {
        ...
        'Directory' {
            ...
            Get-ChildItem -Path $PSTDirectory -Filter *.pst -Recurse
        }
    }
    ...
    $validationResults = foreach ($file in $pstFiles) {
        $validation = Test-PSTFile -FilePath $file.FullName
    ...
    # Prepare AzCopy parameters for directory upload
    $sourcePath = switch ($PSCmdlet.ParameterSetName) {
        'SingleFile' { Split-Path $PSTPath -Parent }
        'Directory' { $PSTDirectory }
    }
    
    # Modified AzCopy parameters to include --from-to
    $azcopyParams = @(
        'copy'
        "`"$sourcePath`""
        "`"$SasUrl`""
        '--from-to=LocalBlob'  # Added this parameter
        '--overwrite=true'
        '--recursive=true'
        '--log-level=INFO'
        '--output-type=json'
...
# Example usage for a directory of PST files
$uploadParams = @{
    PSTDirectory = "D:\ExchangeArchives\Import"  # Directory containing PST files
