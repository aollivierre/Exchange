...
function Export-ArchiveReport {
    ...
    New-HTML @reportParams {
        New-HTMLSection -HeaderText "Archive Enablement Results - $(Get-Date -Format 'yyyy-MM-dd HH:mm')" {
            New-HTMLPanel {
                New-HTMLTable -DataTable $cleanResults {
                    New-TableCondition -Name 'Operation Result' -ComparisonType string -Operator eq -Value 'Failed' -BackgroundColor '#ffcdd2' -Color '#000000'
                    New-TableCondition -Name 'Operation Result' -ComparisonType string -Operator eq -Value 'Enabled' -BackgroundColor '#c8e6c9' -Color '#000000'
                    New-TableCondition -Name 'Operation Result' -ComparisonType string -Operator eq -Value 'Already Enabled' -BackgroundColor '#fff9c4' -Color '#000000'
                } -ScrollX $true -Style center -TextWrap
            }
        } -CanCollapse
    }
}
