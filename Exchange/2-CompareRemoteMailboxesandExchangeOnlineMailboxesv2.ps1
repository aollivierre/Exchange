...
function ARHCompareMailboxes {
    ...
    $remoteMailboxes = Get-RemoteMailbox | Select-Object -Property * -ExcludeProperty PropertyNames, PropertyCount, PropertyLength
    ...
    # Comparing lists to find mailboxes not in Exchange Online
    ...
    # Comparing lists to find mailboxes not in Remote
    $notInRemote = $onlineMailboxes | Where-Object { $_.PrimarySmtpAddress -notin $remoteMailboxes.PrimarySmtpAddress }
    $notInRemoteCount = $notInRemote.Count
    Write-EnhancedLog -Message "Identified $notInRemoteCount mailboxes that are in Exchange Online but not in Remote." -Level "INFO" -ForegroundColor ([ConsoleColor]::Green)

    # Exporting results to GridView and CSV
    if ($notInOnlineCount -gt 0) {
        $notInOnline | Out-GridView -Title "Mailboxes in Remote but not in Exchange Online"
        $notInOnline | Export-Csv -Path "$CSVFilePath\missingMailboxes.csv" -NoTypeInformation -Force
    }

    if ($notInRemoteCount -gt 0) {
        $notInRemote | Out-GridView -Title "Mailboxes in Exchange Online but not in Remote"
        $notInRemote | Export-Csv -Path "$CSVFilePath\extraMailboxes.csv" -NoTypeInformation -Force
    }
    ...
    # Returning both sets of results
    return @{
        NotInOnline = $notInOnline
        NotInRemote = $notInRemote
    }
}

# Example usage of the function
$results = ARHCompareMailboxes -CSVFilePath $CSVFilePath
